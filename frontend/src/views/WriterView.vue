        ...
        <div class="flex-grow overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            <template v-if="currentPhase === 'writing'">
              ...
              <textarea
                v-model="content"
                class="notebook-lines flex-grow w-full p-4 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 resize-none"
              ></textarea>
            </template>
          </div>
...
<style>
/* Style for the notebook lines in the textarea */
.notebook-lines {
  ...
  /* Control line padding with position and size, removing clip */
  background-repeat: no-repeat;
  background-position: 1rem 15px; /* 1rem horizontal padding */
  background-size: calc(100% - 2rem) 100%; /* size gradient to respect padding */
}

/* Dark mode style for the notebook lines */
.dark .notebook-lines {
  background-image: repeating-linear-gradient(
    to bottom,
    transparent,
